...
def plot_metrics(metrics_data, config, display=False):
    ...
    plt.figure(5)
    xLabels = ["Images", "Layer1", "Layer2", "Layer3", "Softmax", "Same Prediction"]
    ...
    plt.axhline(y=metrics_data["val_layer1_dists_max"], label="Layer1 Max. Dist. {:.2f}".format(metrics_data["val_layer1_dists_max"]), color="g", linestyle='dashed', linewidth=1, alpha=0.3)
    plt.axhline(y=metrics_data["val_layer2_dists_max"], label="Layer2 Max. Dist. {:.2f}".format(metrics_data["val_layer2_dists_max"]), color="k", linestyle='dashed', linewidth=1, alpha=0.3)
    plt.axhline(y=metrics_data["val_layer3_dists_max"], label="Layer3 Max. Dist. {:.2f}".format(metrics_data["val_layer3_dists_max"]), color="c", linestyle='dashed', linewidth=1, alpha=0.3)
    plt.axhline(y=metrics_data["val_softmax_output_max"], label="Softmax Max. {:.2f}".format(metrics_data["val_softmax_output_max"]), color="m", linestyle='dashed', linewidth=1, alpha=0.3)
    ...
    for i in range(metrics_data["val_layer1_dists"].shape[0]):
        for j in range(i , metrics_data["val_layer1_dists"].shape[0]):
            ...
            plt.plot(xLabels, layer_plot_sequence, linestyle="solid", marker=".", linewidth=0.5, markeredgewidth=0.1)
